...
//T型中第1个方块是中间那个，以T字型出现
Tshape::Tshape(Grid &grid,Board &bd)
    :Shape(grid,bd)
{
    color = PURPLE;
...
void Tshape::computeRotateCenter()
{
    //调用父类中的函数
    ...
    rotatePositions[FIRST_POSITION][THIRD_GRID]=QPoint(x,y-1);
    rotatePositions[FIRST_POSITION][FOUTH_GRID]=QPoint(x,y+1);

    //第2个方向
    rotatePositions[SECOND_POSITION][FIRST_GRID]=QPoint(x,y-1);
    rotatePositions[SECOND_POSITION][THIRD_GRID]=QPoint(x-1,y);
    rotatePositions[SECOND_POSITION][FOUTH_GRID]=QPoint(x+1,y);

    //第3个方向
    rotatePositions[THIRD_POSITION][FIRST_GRID]=QPoint(x-1,y);
    rotatePositions[THIRD_POSITION][THIRD_GRID]=QPoint(x,y+1);
    rotatePositions[THIRD_POSITION][FOUTH_GRID]=QPoint(x,y-1);

    //第4个方向
    rotatePositions[FOUTH_POSITION][FIRST_GRID]=QPoint(x,y+1);
    rotatePositions[FOUTH_POSITION][THIRD_GRID]=QPoint(x+1,y);
    rotatePositions[FOUTH_POSITION][FOUTH_GRID]=QPoint(x-1,y);
}
